...
// ─── Types ────────────────────────────────────────────────────────────────────
interface DayData { date: string; ts: number; count: number; revenue: number }
type View = 'overview' | 'revenue' | 'sales' | 'growth' | 'tier';

// ─── Mock data ────────────────────────────────────────────────────────────────
const PROMOTER_NAME = 'Tony';
const EVENT_NAME    = 'Championship Boxing — Baltimore 2026';
const EVENT_DATE    = 'Saturday, April 5, 2026';

function buildDays(): DayData[] {
  const counts = [1, 0, 2, 1, 3, 2, 4, 3, 7, 5, 6, 4, 8, 12, 9, 11, 18, 25, 31, 28, 22, 19, 14, 10, 8, 6, 4, 3, 2, 1, 2, 3, 5, 4, 2];
  const price   = 2000;
...
const TOTAL      = ALL_DAYS.reduce((s, d) => s + d.count, 0);    // 161
const GROSS      = ALL_DAYS.reduce((s, d) => s + d.revenue, 0);  // cents
const RATE       = 0.20;
const PAYOUT     = Math.round(GROSS * RATE);
const PLATFORM   = GROSS - PAYOUT;
const AVG        = Math.round(GROSS / TOTAL);
const NEXT_T     = 1001;
const FLOOR      = 100;
const REMAINING  = NEXT_T - TOTAL;
const PROGRESS   = Math.min(
  REMAINING > 0 ? 99 : 100,
  Math.round(((TOTAL - FLOOR) / (NEXT_T - FLOOR)) * 100),
);
const PROJ_CUT   = Math.round(NEXT_T * AVG * 0.30);
const PROJ_DELTA = PROJ_CUT - PAYOUT;
...
const THEME = {
  ...
  tooltip: { container: { background: '#111', border: '1px solid rgba(255,255,255,0.12)', color: '#fff', fontSize: 12, borderRadius: 4 } },
};
...
const TIERS: { range: string; promoter: string; platform: string }[] = [
...
function isTierActive(range: string) {
  if (TOTAL < 100   && range === '0 – 99')       return true;
  if (TOTAL >= 100  && TOTAL <= 1000 && range === '100 – 1,000')   return true;
  if (TOTAL >= 1001 && TOTAL <= 2000 && range === '1,001 – 2,000') return true;
  if (TOTAL >= 2001 && TOTAL <= 3000 && range === '2,001 – 3,000') return true;
  if (TOTAL >= 3001 && TOTAL <= 4000 && range === '3,001 – 4,000') return true;
  if (TOTAL >= 4001 && TOTAL <= 5000 && range === '4,001 – 5,000') return true;
  if (TOTAL >= 5001 && TOTAL <= 6000 && range === '5,001 – 6,000') return true;
  if (TOTAL > 6000  && range === '6,001+')        return true;
  return false;
}
...
function filterDays(days: DayData[], period: Period): DayData[] {
  ...
  const cutoff = new Date(days[days.length - 1].ts);
...
const TOUR_STEPS: TourStep[] = [
  {
    ...
    body: 'This is your real-time promoter report for every event you run with BoxStreamTV. Use the sidebar to navigate between sections. Click Next to take a quick tour.',
  },
  {
    title: 'Overview',
...
export default function ExampleDashboard() {
  ...
  const filtered = filterDays(ALL_DAYS, period);
  const rotate = filtered.length > 10;
  const bottomMargin = rotate ? 56 : 36;

  const barData     = filtered.map((d) => ({ date: d.date, Purchases: d.count }));
  const revenueData = filtered.map((d) => ({ date: d.date, Revenue: d.revenue / 100 }));
  let running = 0;
  const lineData = [{ id: 'Cumulative', data: filtered.map((d) => { running += d.count; return { x: d.date, y: running }; }) }];
  ...
  const GrowthChart = !mounted ? <Skeleton /> : (
    <div className="h-64">
      <ResponsiveLine
        ...
        axisBottom={{ tickSize: 0, tickPadding: 8, tickRotation: rotate ? -45 : 0 }}
        axisLeft={{ tickSize: 0, tickPadding: 8, tickValues: 4, format: (v) => Number.isInteger(v) ? v : '' }}
        gridYValues={4} enablePoints={filtered.length === 1} pointSize={6} pointColor="#fff"
    ...
    <div className="min-h-screen bg-black text-white flex">
      ...
      <aside className="w-52 shrink-0 border-r border-white/10 flex flex-col sticky top-0 h-screen overflow-y-auto">
        ...
        <div className="px-5 pt-4 pb-5 border-b border-white/10">
          ...
          <p className="text-white text-xs font-semibold leading-snug">{EVENT_NAME}</p>
          <p className="text-[10px] text-gray-500 mt-0.5">{EVENT_DATE}</p>
        </div>
        ...
        <div className="px-5 py-4 border-t border-white/10 space-y-2">
          <div className="inline-flex items-center gap-1.5 px-2 py-1 border border-yellow-700/50 bg-yellow-900/10">
            <span className="w-1.5 h-1.5 rounded-full bg-yellow-500" />
            <p className="text-[9px] font-bold tracking-[0.15em] uppercase text-yellow-600">Demo — sample data</p>
          </div>
      ...
      <main className="flex-1 overflow-y-auto px-8 py-10 min-w-0">
        ...
        <div className="mb-10 flex items-end justify-between gap-4">
          <h1 className="text-3xl font-bold tracking-tight">
            <Typewriter text={`Welcome ${PROMOTER_NAME}, to the BoxStreamTV Dashboard.`} />
          </h1>
          {!tourActive && (
            <button
              onClick={startTour}
          ...
          <div className="space-y-10">
            <div>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {([
                  { label: 'Paid Purchases', value: TOTAL.toLocaleString(),  tip: 'Total paid tickets. Complimentary accesses are excluded.' },
                  { label: 'Gross Revenue',  value: fmt(GROSS),              tip: 'Total collected before any fees or splits.' },
                  { label: 'Your Cut',       value: fmt(PAYOUT),             tip: `Your ${Math.round(RATE * 100)}% share of gross at the current tier.` },
                  { label: 'Platform Cut',   value: fmt(PLATFORM),           tip: "BoxStreamTV's share, covering infrastructure, streaming, and payment processing." },
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-zinc-900 border border-white/10 px-5 pt-4 pb-5">
                ...
                <p className="text-[11px] text-gray-500 mt-1">100 – 1,000 purchases</p>
              </div>
              <div className="bg-zinc-900 border border-white/10 px-5 pt-4 pb-5">
                <div className="flex items-center gap-1.5 mb-2">
                ...
                <p className="text-[11px] text-gray-500 mt-1">to unlock 30% ({TOTAL.toLocaleString()} / {NEXT_T.toLocaleString()})</p>
              </div>
            </div>
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="bg-zinc-900 border border-white/10 px-5 pt-4 pb-5">
                ...
                <p className="text-3xl font-bold">{fmt(PAYOUT)}</p>
              </div>
            </div>
            <div>
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="bg-zinc-900 border border-white/10 px-5 pt-4 pb-5">
                ...
                <p className="text-3xl font-bold">{fmt(AVG)}</p>
              </div>
            </div>
            <div>
        ...
        {view === 'growth' && (
          <div className="space-y-8">
            ...
            <div className="bg-zinc-900 border border-white/10 px-5 pt-4 pb-5 inline-block">
              ...
              <p className="text-3xl font-bold">{TOTAL.toLocaleString()} sales</p>
            </div>
            <div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-zinc-900 border border-white/10 px-5 py-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-white font-semibold">100 – 1,000 purchases</p>
              </div>
              ...
              <div className="flex items-start justify-between mb-3">
                <div>
                  ...
                  <p className="text-white font-semibold">
                    {REMAINING.toLocaleString()} more {REMAINING === 1 ? 'sale' : 'sales'} to unlock <span className="font-bold">30%</span>
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-[9px] font-bold tracking-[0.2em] uppercase text-gray-400 mb-0.5">
                    {TOTAL.toLocaleString()} / {NEXT_T.toLocaleString()}
                  </p>
            ...
            <div>
              ...
              <div className="border border-white/10 overflow-hidden">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {TIERS.map((tier) => {
                      const active = isTierActive(tier.range);
                      return (
